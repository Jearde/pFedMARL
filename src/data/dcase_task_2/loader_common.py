...
from data.dcase_task_2.rename_eval_wav import copy_wav as rename_wav
...
DOWNLOAD_PATH_YAML_DICT = {
    "DCASE2025T2": "meta/download_path_2025.yaml",
    "DCASE2024T2": "meta/download_path_2024.yaml",
    "DCASE2023T2": "meta/download_path_2023.yaml",
    "legacy": "meta/download_path_legacy.yaml",
}
########################################################################
...
def download_raw_data(target_dir, dir_name, machine_type, data_type, dataset, root):
    ...
    download_path_yaml = root_dir / download_path_yaml
    ...
    if os.path.exists(f"{target_dir}/{dir_name}") and not os.path.isfile(
        ...
        logger.info(f"{target_dir}/{dir_name} is already downloaded")
        return
    logger.info(f"{target_dir}/{dir_name} is not directory.\nperform dataset download.")
    ...
    for i in np.arange(len(file_url[machine_type][data_type])):
        ...
        try:
            ...
            logger.info(e)
            logger.info("retry dataset download")
            download_raw_data(
    ...
    if data_type == "eval":
        try:
            rename_wav(
                dataset_parent_dir=target_dir.parents[1],
...
def release_write_lock(lock, lock_file_path):
    logger.info(f"{datetime.datetime.now()}\trelease write lock : {lock_file_path}")
    lock.release_write_lock()
    if os.path.isfile(lock_file_path):
        try:
            os.remove(lock_file_path)
        except OSError:
            logger.info(f"can not remove {lock_file_path}")


def release_read_lock(lock, lock_file_path):
    logger.info(f"{datetime.datetime.now()}\trelease read lock : {lock_file_path}")
...
########################################################################
YAML_PATH = {
    "legacy": "meta/machine_type_legacy.yaml",
    "DCASE2023T2_dev": "meta/machine_type_2023_dev.yaml",
    "DCASE2023T2_eval": "meta/machine_type_2023_eval.yaml",
    "DCASE2024T2_dev": "meta/machine_type_2024_dev.yaml",
    "DCASE2024T2_eval": "meta/machine_type_2024_eval.yaml",
    "DCASE2025T2_dev": "meta/machine_type_2025_dev.yaml",
    "DCASE2025T2_eval": "meta/machine_type_2025_eval.yaml",
}


def get_machine_type_dict(dataset_name, mode=True):
    if isinstance(mode, str):
        mode = True if mode == "dev" else False
    ...
    yaml_path = current_file_path / yaml_path
